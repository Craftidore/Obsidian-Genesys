# Genesys Dice & Symbols for Obsidian

I love the Gensys Narrative Dice System, but unlike normal dice, which are easily abreviated with `dXX`, there hasn't been a good way of referencing these dice in plaintext. 

This CSS Snippet changes tags of the form `#Genesys<die or symbol name>` into the image of that die or symbol. In addition to adding pretty #GenesysBoost NDS #GenesysAbility symbols #GenesysProficiency into your documents, this simplifies writing about Gensys because tags autocomplete.  

| Tag             | Result |
|-----------------|--------|
| `#GenesysAbility`   | #GenesysAbility |
| `#GenesysBoost`     | #GenesysBoost |
| `#GenesysChallenge` | #GenesysChallenge |
| `#GenesysDifficulty` | #GenesysDifficulty |
| `#GenesysProficiency` | #GenesysProficiency |
| `#GenesysSetback`   | #GenesysSetback |
| `#GenesysAdvantage` | #GenesysAdvantage |
| `#GenesysDespair`   | #GenesysDespair |
| `#GenesysFailure`   | #GenesysFailure |
| `#GenesysSuccess`   | #GenesysSuccess |
| `#GenesysThreat`    | #GenesysThreat |
| `#GenesysTriumph`   | #GenesysTriumph |

The CSS in this snippet was adapted from [this](https://www.reddit.com/r/genesysrpg/comments/a92p12/gm_binder_genesys_dice_and_symbols_style/) reddit post. 

```css
/* Style Settings comment for use with the Style Settings Community Plugin */
/* @settings

name: Genesys Dice CSS Snippet
id: craftidore-genesys-dice-snippet
settings:
    - 
        id: do-not-invert-symbols
        title: "Don't Invert Symbols"
		description: Don't invert the color of the symbols while in dark theme. 
        type: class-toggle
		default: false
*/

/* Apllies to Everything */ 
.tag[href^="#GenesysAbility"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysAbility:not(.cm-formatting),
.tag[href^="#GenesysBoost"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysBoost:not(.cm-formatting),
.tag[href^="#GenesysChallenge"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysChallenge:not(.cm-formatting),
.tag[href^="#GenesysDifficulty"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysDifficulty:not(.cm-formatting),
.tag[href^="#GenesysProficiency"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysProficiency:not(.cm-formatting),
.tag[href^="#GenesysSetback"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysSetback:not(.cm-formatting),
.tag[href^="#GenesysAdvantage"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysAdvantage:not(.cm-formatting),
.tag[href^="#GenesysDespair"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysDespair:not(.cm-formatting),
.tag[href^="#GenesysFailure"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysFailure:not(.cm-formatting),
.tag[href^="#GenesysSuccess"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysSuccess:not(.cm-formatting),
.tag[href^="#GenesysThreat"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysThreat:not(.cm-formatting),
.tag[href^="#GenesysTriumph"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysTriumph:not(.cm-formatting) {
	display: inline-flex;
	align-self: center;
	top: .175em;
	position: relative;
	height: 1em;
	width: 1em;
	border: 0;
}

/* Because I don't want the `#` showing up in the tag */
.is-live-preview .cm-line:not(.cm-active) .cm-formatting.cm-tag-GenesysAbility,
.is-live-preview .cm-line:not(.cm-active) .cm-formatting.cm-tag-GenesysBoost,
.is-live-preview .cm-line:not(.cm-active) .cm-formatting.cm-tag-GenesysChallenge,
.is-live-preview .cm-line:not(.cm-active) .cm-formatting.cm-tag-GenesysDifficulty,
.is-live-preview .cm-line:not(.cm-active) .cm-formatting.cm-tag-GenesysProficiency,
.is-live-preview .cm-line:not(.cm-active) .cm-formatting.cm-tag-GenesysSetback,
.is-live-preview .cm-line:not(.cm-active) .cm-formatting.cm-tag-GenesysAdvantage,
.is-live-preview .cm-line:not(.cm-active) .cm-formatting.cm-tag-GenesysDespair,
.is-live-preview .cm-line:not(.cm-active) .cm-formatting.cm-tag-GenesysFailure,
.is-live-preview .cm-line:not(.cm-active) .cm-formatting.cm-tag-GenesysSuccess,
.is-live-preview .cm-line:not(.cm-active) .cm-formatting.cm-tag-GenesysThreat,
.is-live-preview .cm-line:not(.cm-active) .cm-formatting.cm-tag-GenesysTriumph {
	display: none;
}

/* Dice */

.tag[href^="#GenesysAbility"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysAbility:not(.cm-formatting) {
	content: url("data:image/svg+xml;base64,");
}

.tag[href^="#GenesysBoost"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysBoost:not(.cm-formatting) {
	padding-right: 1px;
	padding-left: 1px;
	content: url("data:image/svg+xml;base64,");
}

.tag[href^="#GenesysChallenge"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysChallenge:not(.cm-formatting) {
	content: url("data:image/svg+xml;base64,");
}

.tag[href^="#GenesysDifficulty"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysDifficulty:not(.cm-formatting) {
	content: url("data:image/svg+xml;base64,");
}

.tag[href^="#GenesysProficiency"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysProficiency:not(.cm-formatting) {
	content: url("data:image/svg+xml;base64,");
}

.tag[href^="#GenesysSetback"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysSetback:not(.cm-formatting) {
	padding-right: 1px;
	padding-left: 1px;
	content: url("data:image/svg+xml;base64,");
}

/* Symbols */
.tag[href^="#GenesysAdvantage"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysAdvantage:not(.cm-formatting) {
	content: url("data:image/svg+xml;base64,");
}

.tag[href^="#GenesysDespair"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysDespair:not(.cm-formatting) {
	content: url("data:image/svg+xml;base64,");
}

.tag[href^="#GenesysFailure"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysFailure:not(.cm-formatting) {
	content: url("data:image/svg+xml;base64,");
}

.tag[href^="#GenesysSuccess"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysSuccess:not(.cm-formatting) {
	content: url("data:image/svg+xml;base64,");
}

.tag[href^="#GenesysThreat"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysThreat:not(.cm-formatting) {
	content: url("data:image/svg+xml;base64,");
}

.tag[href^="#GenesysTriumph"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysTriumph:not(.cm-formatting) {
	content: url("data:image/svg+xml;base64,");
}

/* Dark-theme fixes. Can be toggled off with Style Settings plugin */
.theme-dark:not(.do-not-invert-symbols) .tag[href^="#GenesysAdvantage"], .theme-dark:not(.do-not-invert-symbols) .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysAdvantage:not(.cm-formatting), 
.theme-dark:not(.do-not-invert-symbols) .tag[href^="#GenesysDespair"], .theme-dark:not(.do-not-invert-symbols) .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysDespair:not(.cm-formatting), 
.theme-dark:not(.do-not-invert-symbols) .tag[href^="#GenesysFailure"], .theme-dark:not(.do-not-invert-symbols) .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysFailure:not(.cm-formatting), 
.theme-dark:not(.do-not-invert-symbols) .tag[href^="#GenesysSuccess"], .theme-dark:not(.do-not-invert-symbols) .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysSuccess:not(.cm-formatting), 
.theme-dark:not(.do-not-invert-symbols) .tag[href^="#GenesysThreat"], .theme-dark:not(.do-not-invert-symbols) .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysThreat:not(.cm-formatting), 
.theme-dark:not(.do-not-invert-symbols) .tag[href^="#GenesysTriumph"], .theme-dark:not(.do-not-invert-symbols) .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysTriumph:not(.cm-formatting) {
	filter: invert(1);
}

/* css here to 
A: make the square dice less flat-seeming 
B: make sure the setback die can be seen in dark mode
*/
.tag[href^="#GenesysSetback"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysSetback:not(.cm-formatting) {
	background-color: #686868;
	border-radius: 1px;
}
.tag[href^="#GenesysBoost"], .is-live-preview .cm-line:not(.cm-active) .cm-tag-GenesysBoost:not(.cm-formatting) {
	background-color: #989898;
	border-radius: 1px;
}
```